...
ClassificationNetwork* ClassificationNetwork_create(int layerAmount, int* layersSize, int input_dim, ActivationType* activations, double learnningRate, LossType lossFunction, char** class_names, int num_classes, Tensor* classes)
{
    ...
    Cnet->net = network_create(layerAmount,layersSize,input_dim,activations,learnningRate, lossFunction); //cant free the network
    if(!Cnet->net){
        fprintf(stderr, "Error: Memory allocation failed for network in ClassificationNetwork_create_net\n");
        return NULL;
    }
